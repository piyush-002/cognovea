...
const PHASES: { id: string; label: string; h: string; paras: string[]; block?: Block }[] = [
  {
    id: 'phase-1',
    label: '01 · Assessment',
  ...
  },
  {
    ...
    block: {
      h: 'Language Models, Prediction Tools and Custom Architecture',
      items: [
        {
            ...
            items={PHASES.map((p) => ({
              ...
              content: (
                <>
                  ...
                  {p.block && (
                    <div className="card card--flat" style={{ marginTop: '1.8rem' }}>
                      <h4 className="h-sm">{p.block.h}</h4>
                      {p.block.paras?.map((t, i) => (
                        <p key={i}>{t}</p>
                      ))}
                      {p.block.items && (
                        <ul className="rich" style={{ listStyle: 'none', padding: 0, marginTop: '1.1rem' }}>
                          {p.block.items.map((it) => (
                            <li
                              key={it.t}
